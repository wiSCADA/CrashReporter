...
int main( int argc, char* argv[] )
{
    ...
    Config crashReporterConfig = {
		...
		QLatin1String(wiSCADA::Constants::IDE_VERSION_LONG),
        QString(argv[1])
    };

    QApplication::setApplicationName(QString("%1 Crash Reporter").arg(crashReporterConfig.productName));
    QApplication::setApplicationVersion(QLatin1String(wiSCADA::Constants::IDE_VERSION_LONG));
    ...
    window.show();
	QMessageBox::information(&window,"", crashReporterConfig.minidumpPath);
    return app.exec();
}
